...
describe("ValidateInput", () => {
    ...
    describe("isInputEmpty", () => {
        it("should be able validate if input is empty", () => {
            ...
            const spy = jest.spyOn(validateInput, "isInputEmpty").mockImplementationOnce(() => {
          ...
          it("should be able validate if input is not empty", () => {
            ...
            const spy = jest.spyOn(validateInput, "isInputEmpty").mockReturnValueOnce(false);
    ...
    describe("isBinary", () => {
      it("should be able validate if input is not binary", () => {
        ...
        const spy = jest.spyOn(validateInput, "isBinary").mockImplementationOnce(() => {
            throw new Error("Input is not a binary")
        })
    
        expect(() => validateInput.isBinary()).toThrowError("Input is not a binary");
        expect(spy).toHaveBeenCalledTimes(1)
      });
    
      it("should be able validate if input is a binary without comma", () => {
        const validateInput = new ValidateInput("101010");
        const spy = jest.spyOn(validateInput, "isBinary").mockReturnValueOnce(true);
